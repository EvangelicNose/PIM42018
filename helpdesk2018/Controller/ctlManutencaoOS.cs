...
namespace helpdesk2018.Controller
{
    public static class ctlManutencaoOS
    {
        ...
        public static DataTable ListarOS()
        {
            ...
            {
                SQL = "SELECT " +
                    "tb_chamados.os AS OS, " +
                    "tb_empresas.nome AS EMPRESA, " +
                    "tb_motivos.descricao AS CAUSA," +
                    " tb_status.descricao AS STATUS," +
                    " tb_chamados.criado_em AS ABERTO, " +
                    " tb_usuarios.usuario AS RESPONSÁVEL " +
                    "FROM (tb_empresas INNER JOIN tb_usuarios ON tb_empresas.idempresa = tb_usuarios.fk_idempresa) " +

                    "INNER JOIN(tb_status INNER JOIN (tb_motivos INNER JOIN tb_chamados " +
